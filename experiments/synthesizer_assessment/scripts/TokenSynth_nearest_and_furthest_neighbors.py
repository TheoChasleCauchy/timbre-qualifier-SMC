...
def compute_nearest_neighbors():
    """
    Compute and save the nearest neighbor audio samples for each instrument and condition type.

    This function:
    1. For each condition type, loads the predictions and ground truth data.
    2. For each instrument, finds the synthesized sample closest to the ground truth.
    3. Saves the nearest neighbor audio and its spectrogram.

    Steps:
    - For each condition type, load the predictions and ground truth.
    - For each instrument, compute the Euclidean distance between each synthesized sample and the ground truth.
    ...
    for condition_type in tqdm(condition_types, desc="Processing nearest neighbors for 3 condition types", total=len(condition_types)):
        ...
        output_folder = f"experiments/synthesizer_assessment/results/nearest_and_furthest_neighbors/TokenSynth/{condition_type}_conditioned_synthesis/nearest_neighbors_over_all_traits"
        ...
        for instrument in tqdm(instrument_names, desc=f"Processing nearest neighbors"):
            ...
            audio_path = os.path.join("data/TokenSynth/Samples/", condition_type + "_conditioned_synthesis", instrument, instrument + "_" + basename)
...
def compute_nearest_neighbor_each_trait():
    """
    Compute and save the nearest neighbor audio samples for each instrument, condition type, and timber trait.

    This function:
    1. For each condition type, loads the predictions and ground truth data.
    2. For each instrument and timber trait, finds the synthesized sample closest to the ground truth.
    3. Saves the nearest neighbor audio and its spectrogram.

    Steps:
    - For each condition type, load the predictions and ground truth.
    - For each instrument and timber trait, compute the Euclidean distance between each synthesized sample and the ground truth.
    - Save the nearest neighbor audio and its spectrogram to disk.
    ...
    """
    condition_types = ["text", "audio", "text_audio"]
    for condition_type in tqdm(condition_types, desc="Processing nearest neighbors (each trait) for 3 condition types", total=len(condition_types)):
        # Load predictions metadata
        metadata_path = f"experiments/synthesizer_assessment/results/{condition_type}_conditioned_synthesis/{condition_type}_predictions.csv"
        metadata_df = pd.read_csv(metadata_path)
        ...
        for instrument in tqdm(instrument_names, desc=f"Processing nearest neighbors (each trait)"):
            ...
            # Create output directory
            output_folder = f"experiments/synthesizer_assessment/results/nearest_and_furthest_neighbors/TokenSynth/{condition_type}_conditioned_synthesis/nearest_neighbors_each_trait/{instrument.replace(' ', '_')}"
            os.makedirs(output_folder, exist_ok=True)
...
def compute_furthest_neighbors():
    """
    Compute and save the furthest neighbor audio samples for each instrument and condition type.

    This function:
    1. For each condition type, loads the predictions and ground truth data.
    2. For each instrument, finds the synthesized sample furthest from the ground truth.
    ...
    """
    condition_types = ["text", "audio", "text_audio"]
    for condition_type in tqdm(condition_types, desc="Processing furthest neighbors for 3 condition types", total=len(condition_types)):
        # Load predictions metadata
        metadata_path = f"experiments/synthesizer_assessment/results/{condition_type}_conditioned_synthesis/{condition_type}_predictions.csv"
        metadata_df = pd.read_csv(metadata_path)
        ...
        # Create output directory
        output_folder = f"experiments/synthesizer_assessment/results/nearest_and_furthest_neighbors/TokenSynth/{condition_type}_conditioned_synthesis/furthest_neighbors_over_all_traits"
        os.makedirs(output_folder, exist_ok=True)
...
def compute_furthest_neighbor_each_trait():
    """
    Compute and save the furthest neighbor audio samples for each instrument, condition type, and timber trait.

    This function:
    1. For each condition type, loads the predictions and ground truth data.
    2. For each instrument and timber trait, finds the synthesized sample furthest from the ground truth.
    3. Saves the furthest neighbor audio and its spectrogram.
    ...
    """
    condition_types = ["text", "audio", "text_audio"]
    for condition_type in tqdm(condition_types, desc="Processing furthest neighbors (each trait) for 3 condition types", total=len(condition_types)):
        # Load predictions metadata
        metadata_path = f"experiments/synthesizer_assessment/results/{condition_type}_conditioned_synthesis/{condition_type}_predictions.csv"
        metadata_df = pd.read_csv(metadata_path)
        ...
        for instrument in tqdm(instrument_names, desc=f"Processing furthest neighbors (each trait)"):
            ...
            # Create output directory
            output_folder = f"experiments/synthesizer_assessment/results/nearest_and_furthest_neighbors/TokenSynth/{condition_type}_conditioned_synthesis/furthest_neighbors_each_trait/{instrument.replace(' ', '_')}"
            os.makedirs(output_folder, exist_ok=True)
...
def tokenSynth_nearest_and_furthest_neighbors():
    """
    Compute and save the nearest and furthest neighbor audio samples for all condition types.

    This function calls the individual functions to compute and save the nearest and furthest neighbors.

    Steps:
    - Call functions to compute and save nearest and furthest neighbors for all condition types.
